...
public class Player : MonoBehaviour
{
    ...
    public void toggleTool(int toolId) //N.B: il player deve avere la mano libera per prender in mano l'utensile 
    {
        ...
        {
            equippedToolId = toolId; //l'id del tool che andr� ad equipaggiare = toolId
            hasObjectInHand = true;
        }
        else if (equippedToolId != -1) //se invece ho qualcosa in mano ed � un utensile
        {
            ...
            if (equippedToolId == toolId) //se � lo stesso che avevo prima
            {
